...
if __name__ == '__main__':
    ...
    image_path = './data_test/image'
    label_path = './data_test/label'
    number = 0
    ...
    for ii, sample in enumerate(dataloader):
        for jj in range(sample["image"].size()[0]):
            ...
            cv2.imwrite(os.path.join(image_path, str(number).zfill(6) + '.jpg'), img_tmp)
            cv2.imwrite(os.path.join(label_path, str(number).zfill(6) + '.jpg'), segmap * 255)
            number += 1
